...
export default function SigiloX() {
  ...
  const [currentChatSlide, setCurrentChatSlide] = useState(0)
...
{ code: "+998", name: "Uzbekistan", flag: "🇺🇿", placeholder: "90 123 45 67" }
  ...
  // Updated blocked images with new screenshots
const blockedImages = [
  "https://hebbkx1anhila5yf.public.blob.vercel-storage.com/WhatsApp%20Image%202025-08-07%20at%2009.18.58%20%281%29-SDgJmzuYK8OHJXAJIZjt6ZhVv24afZ.jpeg",
  "https://hebbkx1anhila5yf.public.blob.vercel-storage.com/WhatsApp%20Image%202025-08-07%20at%2009.19.48-iYV2Kn7QZbnpRblJJQIKwk1ocJf97i.jpeg", 
  "https://hebbkx1anhila5yf.public.blob.vercel-storage.com/WhatsApp%20Image%202025-08-07%20at%2009.18.58-fmK8aa5tVlyB2lWXVQcbZxtFtD97Y3.jpeg",
]

  // Chat carousel images
  const chatImages = [
    "https://hebbkx1anhila5yf.public.blob.vercel-storage.com/WhatsApp%20Image%202025-08-07%20at%2008.48.50%20%282%29-MdhOUip7j2Nay1oCfCBexqwZHlOx8r.jpeg",
    "https://hebbkx1anhila5yf.public.blob.vercel-storage.com/WhatsApp%20Image%202025-08-07%20at%2008.48.50-xQLpHUOjwty8uxI1qdaCwrrtheHkJ2.jpeg", 
    "https://hebbkx1anhila5yf.public.blob.vercel-storage.com/WhatsApp%20Image%202025-08-07%20at%2008.48.50%20%281%29-ycteLdZipWdXD4Wo1yMpGOuUsCPgeO.jpeg"
  ]
  ...
  const nextChatSlide = () => {
    setCurrentChatSlide((prev) => (prev + 1) % chatImages.length)
  }

  const prevChatSlide = () => {
    setCurrentChatSlide((prev) => (prev - 1 + chatImages.length) % chatImages.length)
  }
  ...
  // Auto-scroll for chat carousel
  useEffect(() => {
    if (currentStep === "result") {
      const interval = setInterval(nextChatSlide, 4000)
      return () => clearInterval(interval)
    }
  }, [currentStep])
          ...
          {currentStep === "result" && (
            ...
            >
              ...
              <div className="container mx-auto max-w-4xl">
                ...
                {/* NEW: Censored Tinder Chats Section */}
                <Card className="bg-white rounded-2xl shadow-lg border-0 mb-6 sm:mb-8">
                  <CardContent className="p-4 sm:p-6">
                    <h3 className="text-lg sm:text-xl font-bold text-[#333333] mb-4 sm:mb-6">💬 CENSORED TINDER CHATS</h3>
                    <p className="text-sm sm:text-base text-gray-600 mb-4 sm:mb-6">
                      Preview of real conversations exchanged – including hidden messages.
                    </p>
                    ...
                    <div className="relative">
                      <div className="overflow-hidden rounded-xl shadow-lg">
                        <div
                          className="flex transition-transform duration-300 ease-in-out"
                          style={{ transform: `translateX(-${currentChatSlide * 100}%)` }}
                        >
                          {chatImages.map((image, index) => (
                            <div key={index} className="w-full flex-shrink-0 relative aspect-video">
                              <img
                                src={image || "/placeholder.svg"}
                                alt={`Censored chat conversation ${index + 1}`}
                              ...
                              <div className="absolute inset-0 bg-black bg-opacity-30 flex items-center justify-center rounded-xl">
                                <div className="text-center">
                                  <Lock className="w-8 h-8 sm:w-10 sm:h-10 text-white mx-auto mb-2 opacity-90" />
                                  <p className="text-white text-sm font-bold opacity-90">CENSORED</p>
                                </div>
                              </div>
                            </div>
                          ))}
                      ...
                      {/* Chat Dots Indicator */}
                      <div className="flex justify-center gap-2 mt-4">
                        {chatImages.map((_, index) => (
                          <button
                            key={index}
                            onClick={() => setCurrentChatSlide(index)}
                            className={`w-2 h-2 sm:w-3 sm:h-3 rounded-full transition-colors ${
                              index === currentChatSlide ? "bg-[#E91E63]" : "bg-gray-300"
                            }`}
                          />
                        ))}
                      </div>
